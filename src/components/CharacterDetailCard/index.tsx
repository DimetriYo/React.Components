...
export function CharacterDetailCard() {
  ...
  const handleClickWhenCardShown = (e: MouseEvent) => {
    ...
    const isOpenCardClick = !!(e.target as HTMLElement).closest(
      '#character-details'
    );
    const isNewCardClick = !!(e.target as HTMLElement).closest(
      '[data-character-card]'
    );
    if (!isOpenCardClick && !isNewCardClick && clickCounterRef.current > 1)
      hideDetails();
  };
